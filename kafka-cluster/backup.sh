...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/kafka-cluster/etc" ]; then
        cp -r /opt/kafka-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/kafka-cluster" ]; then
        cp -r /etc/kafka-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
