...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/rabbitmq-cluster/etc" ]; then
        cp -r /opt/rabbitmq-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/rabbitmq-cluster" ]; then
        cp -r /etc/rabbitmq-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
