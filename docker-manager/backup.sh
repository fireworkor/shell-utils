...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/docker-manager/etc" ]; then
        cp -r /opt/docker-manager/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/docker-manager" ]; then
        cp -r /etc/docker-manager "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
