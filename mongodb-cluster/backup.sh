...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/mongodb-cluster/etc" ]; then
        cp -r /opt/mongodb-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/mongodb-cluster" ]; then
        cp -r /etc/mongodb-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
