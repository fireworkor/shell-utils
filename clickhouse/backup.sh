...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/clickhouse/etc" ]; then
        cp -r /opt/clickhouse/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/clickhouse" ]; then
        cp -r /etc/clickhouse "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
