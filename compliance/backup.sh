...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/compliance/etc" ]; then
        cp -r /opt/compliance/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/compliance" ]; then
        cp -r /etc/compliance "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
