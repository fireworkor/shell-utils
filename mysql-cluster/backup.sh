...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/mysql-cluster/etc" ]; then
        cp -r /opt/mysql-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/mysql-cluster" ]; then
        cp -r /etc/mysql-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
