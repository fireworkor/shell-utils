...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/cloud-manager/etc" ]; then
        cp -r /opt/cloud-manager/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/cloud-manager" ]; then
        cp -r /etc/cloud-manager "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
