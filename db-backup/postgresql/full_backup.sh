...
LOG_FILE="$BACKUP_ROOT/postgresql_full_backup.log"
...
do_full_backup() {
    ...
    # 使用 pg_dump 导出所有数据库
    su - "$PG_USER" -c "pg_dumpall -h $PG_HOST -p $PG_PORT -f $FULL_DIR/full_$timestamp.sql" 2>/dev/null
    
    if [ $? -eq 0 ] && [ -s "$FULL_DIR/full_$timestamp.sql" ]; then
        # 压缩备份
        gzip "$FULL_DIR/full_$timestamp.sql"
...
EOF
        ...
        log_error "全量备份失败"
        rm -f "$FULL_DIR/full_$timestamp.sql"
        return 1
    fi
}
...
main() {
    ...
    case "$1" in
        backup)
            ...
            ;;
        verify)
            local latest=$(ls -t "$FULL_DIR"/full_*.tar.gz 2>/dev/null | head -1)
            if [ -n "$latest" ]; then
