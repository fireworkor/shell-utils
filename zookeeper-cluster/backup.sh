...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/zookeeper-cluster/etc" ]; then
        cp -r /opt/zookeeper-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/zookeeper-cluster" ]; then
        cp -r /etc/zookeeper-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
