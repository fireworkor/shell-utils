...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/cost-optimizer/etc" ]; then
        cp -r /opt/cost-optimizer/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/cost-optimizer" ]; then
        cp -r /etc/cost-optimizer "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
