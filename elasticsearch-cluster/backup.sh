...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/elasticsearch-cluster/etc" ]; then
        cp -r /opt/elasticsearch-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/elasticsearch-cluster" ]; then
        cp -r /etc/elasticsearch-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
...
list_backups() {
    echo "Elasticsearch Cluster 的备份列表:"
