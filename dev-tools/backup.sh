...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/dev-tools/etc" ]; then
        cp -r /opt/dev-tools/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/dev-tools" ]; then
        cp -r /etc/dev-tools "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
