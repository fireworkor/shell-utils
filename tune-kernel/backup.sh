...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/tune-kernel/etc" ]; then
        cp -r /opt/tune-kernel/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/tune-kernel" ]; then
        cp -r /etc/tune-kernel "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
