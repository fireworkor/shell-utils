...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/redis-cluster/etc" ]; then
        cp -r /opt/redis-cluster/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/redis-cluster" ]; then
        cp -r /etc/redis-cluster "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
