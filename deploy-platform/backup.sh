...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/deploy-platform/etc" ]; then
        cp -r /opt/deploy-platform/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/deploy-platform" ]; then
        cp -r /etc/deploy-platform "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
