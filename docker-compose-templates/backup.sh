...
do_backup() {
    ...
    # 备份配置
    if [ -d "/opt/docker-compose-templates/etc" ]; then
        cp -r /opt/docker-compose-templates/etc "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    elif [ -d "/etc/docker-compose-templates" ]; then
        cp -r /etc/docker-compose-templates "$backup_dir/" 2>/dev/null
        echo -e "${GREEN}配置备份完成${NC}"
    fi
...
list_backups() {
    echo "Docker Compose Templates 的备份列表:"
