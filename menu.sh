...
show_main_menu() {
    ...
    echo ""
    echo "  ${GREEN}1.${NC} 软件安装"
    echo "  ${GREEN}2.${NC} 集群部署"
    echo "  ${GREEN}3.${NC} 运维工具"
    echo "  ${GREEN}4.${NC} 安全扫描"
    echo "  ${GREEN}5.${NC} 日常巡检"
    echo "  ${GREEN}6.${NC} 备份恢复"
    echo "  ${GREEN}7.${NC} 远程部署"
    echo "  ${GREEN}8.${NC} 脚本升级"
    echo "  ${GREEN}9.${NC} Web UI"
    echo "  ${GREEN}A.${NC} 多云管理"
    echo "  ${GREEN}B.${NC} 自动化部署"
    echo "  ${GREEN}C.${NC} 成本优化"
    echo "  ${GREEN}D.${NC} 合规检查"
    echo "  ${GREEN}E.${NC} PVE虚拟机部署"
    echo "  ${GREEN}0.${NC} 退出"
    echo ""
...
show_install_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择要安装的软件：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Web 服务器"
    echo "  ${GREEN}2.${NC} 数据库"
    echo "  ${GREEN}3.${NC} 编程语言"
    echo "  ${GREEN}4.${NC} 容器和缓存"
    echo "  ${GREEN}5.${NC} 大数据组件"
    echo "  ${GREEN}6.${NC} 一键部署"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_web_server_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择 Web 服务器：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Nginx"
    echo "  ${GREEN}2.${NC} Apache"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_database_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择数据库：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} MySQL"
    echo "  ${GREEN}2.${NC} MariaDB"
    echo "  ${GREEN}3.${NC} PostgreSQL"
    echo "  ${GREEN}4.${NC} MongoDB"
    echo "  ${GREEN}5.${NC} Redis"
    echo "  ${GREEN}6.${NC} Elasticsearch"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
show_language_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择编程语言：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} PHP"
    echo "  ${GREEN}2.${NC} Python"
    echo "  ${GREEN}3.${NC} Node.js"
    echo "  ${GREEN}4.${NC} Java"
    echo "  ${GREEN}5.${NC} Go"
    echo "  ${GREEN}6.${NC} Rust"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
show_container_menu() {
    ...
    echo "${YELLOW}请选择：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Docker"
    echo "  ${GREEN}2.${NC} Redis"
    echo "  ${GREEN}3.${NC} Memcached"
    echo "  ${GREEN}4.${NC} RabbitMQ"
    echo "  ${GREEN}5.${NC} Kafka"
    echo "  ${GREEN}6.${NC} MinIO"
    echo "  ${GREEN}7.${NC} 消息队列"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_messaging_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择消息队列：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} NATS (高性能消息系统)"
    echo "  ${GREEN}2.${NC} Apache Pulsar (云原生消息队列)"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-2]: " choice
...
show_cloud_menu() {
    ...
    echo "${YELLOW}请选择操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 列出账户"
    echo "  ${GREEN}2.${NC} 添加账户"
    echo "  ${GREEN}3.${NC} 删除账户"
    echo "  ${GREEN}4.${NC} 设置默认账户"
    echo "  ${GREEN}5.${NC} 列出所有实例"
    echo "  ${GREEN}6.${NC} 查看所有费用"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
execute_deploy() {
    ...
    echo "${YELLOW}请选择操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 列出已部署应用"
    echo "  ${GREEN}2.${NC} 部署新应用"
    echo "  ${GREEN}3.${NC} 扩缩容"
    echo "  ${GREEN}4.${NC} 查看状态"
    echo "  ${GREEN}5.${NC} 回滚"
    echo "  ${GREEN}6.${NC} 删除应用"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
execute_cost() {
    ...
    echo "${YELLOW}请选择操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 成本分析"
    echo "  ${GREEN}2.${NC} 生成报告"
    echo "  ${GREEN}3.${NC} 右尺寸建议"
    echo "  ${GREEN}4.${NC} 闲置资源"
    echo "  ${GREEN}5.${NC} 设置预算"
    echo "  ${GREEN}6.${NC} 执行优化"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
execute_compliance() {
    ...
    echo "${YELLOW}请选择操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} CIS Benchmark 检查"
    echo "  ${GREEN}2.${NC} PCI DSS 检查"
    echo "  ${GREEN}3.${NC} HIPAA 检查"
    echo "  ${GREEN}4.${NC} GDPR 检查"
    echo "  ${GREEN}5.${NC} SOC 2 检查"
    echo "  ${GREEN}6.${NC} 查看规则"
    echo "  ${GREEN}7.${NC} 生成报告"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-7]: " choice
...
show_bigdata_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择大数据组件：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Hadoop"
    echo "  ${GREEN}2.${NC} Spark"
    echo "  ${GREEN}3.${NC} Flink"
    echo "  ${GREEN}4.${NC} HBase"
    echo "  ${GREEN}5.${NC} Hive"
    echo "  ${GREEN}6.${NC} Airflow"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
show_oneclick_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择一键部署方案：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} LNMP 栈 (Nginx + MySQL + PHP)"
    echo "  ${GREEN}2.${NC} LAMP 栈 (Apache + MySQL + PHP)"
    echo "  ${GREEN}3.${NC} 开发工具"
    echo "  ${GREEN}4.${NC} 大数据组件栈"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_cluster_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择集群类型：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Redis 集群"
    echo "  ${GREEN}2.${NC} MySQL 集群"
    echo "  ${GREEN}3.${NC} MongoDB 集群"
    echo "  ${GREEN}4.${NC} PostgreSQL 集群"
    echo "  ${GREEN}5.${NC} Kafka 集群"
    echo "  ${GREEN}6.${NC} Elasticsearch 集群"
    echo "  ${GREEN}7.${NC} Kubernetes 工具"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-7]: " choice
...
show_k8s_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择 Kubernetes 工具：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} Argo CD (GitOps)"
    echo "  ${GREEN}2.${NC} Jaeger (分布式追踪)"
    echo "  ${GREEN}3.${NC} Istio (服务网格)"
    echo "  ${GREEN}4.${NC} Linkerd (服务网格)"
    echo "  ${GREEN}5.${NC} Kong (API 网关)"
    echo "  ${GREEN}6.${NC} Apache APISIX (API 网关)"
    echo "  ${GREEN}7.${NC} Consul (服务发现)"
    echo "  ${GREEN}8.${NC} Vault (密钥管理)"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_ops_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择运维工具：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 内核调优"
    echo "  ${GREEN}2.${NC} 防火墙管理"
    echo "  ${GREEN}3.${NC} 镜像源设置"
    echo "  ${GREEN}4.${NC} 系统清理"
    echo "  ${GREEN}5.${NC} 健康检查"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
...
show_security_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择安全检查类型：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 完整安全扫描"
    echo "  ${GREEN}2.${NC} 操作系统安全"
    echo "  ${GREEN}3.${NC} 数据库安全"
    echo "  ${GREEN}4.${NC} Web 服务器安全"
    echo "  ${GREEN}5.${NC} Docker 安全"
    echo "  ${GREEN}6.${NC} 网络安全"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-6]: " choice
...
show_backup_menu() {
    ...
    echo "${YELLOW}请选择操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} MySQL 备份"
    echo "  ${GREEN}2.${NC} MongoDB 备份"
    echo "  ${GREEN}3.${NC} Redis 备份"
    echo "  ${GREEN}4.${NC} 全部备份"
    echo "  ${GREEN}5.${NC} MySQL 恢复"
    echo "  ${GREEN}6.${NC} MongoDB 恢复"
    echo "  ${GREEN}7.${NC} Redis 恢复"
    echo "  ${GREEN}0.${NC} 返回"
    echo ""
    read -p "请输入选项 [0-7]: " choice
...
show_pve_menu() {
    ...
    echo ""
    echo "${YELLOW}请选择PVE操作：${NC}"
    echo ""
    echo "  ${GREEN}1.${NC} 配置PVE连接"
    echo "  ${GREEN}2.${NC} 查看当前配置"
    echo "  ${GREEN}3.${NC} 列出所有VM"
    echo "  ${GREEN}4.${NC} 创建新VM（配置文件）"
    echo "  ${GREEN}5.${NC} 创建新VM（交互式）"
    echo "  ${GREEN}6.${NC} 启动VM"
    echo "  ${GREEN}7.${NC} 停止VM"
    echo "  ${GREEN}8.${NC} 查看VM状态"
    echo "  ${GREEN}9.${NC} 部署软件到VM"
    echo "  ${GREEN}10.${NC} 销毁VM"
